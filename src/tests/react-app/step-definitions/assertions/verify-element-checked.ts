...
import { getElementClass } from '../../../../resources/common/html-behavior';
...
Then(
    /^the( ([0-9]+st|[0-9]+nd|[0-9]+rd|[0-9]+th))? "([^"]*)"( [a-z]*)? should (not )?be mui-checked$/,
    async function(
        this: ScenarioWorld,
        elementPosition: string,
        elementKey: ElementKey,
        ...
        console.log(`The ${elementPosition?elementPosition+' ':''}${elementKey}${elementType?elementType:''} should ${negate?'not ':''}be mui-checked`);
        ...
        await waitFor( async () => {
            const elementClass: string | null = await getElementClass(page, elementIdentifier);
            console.log(`elementClass 2 = ${elementClass}`);
            return elementClass?.includes('Mui-checked') === !negate;
        });
